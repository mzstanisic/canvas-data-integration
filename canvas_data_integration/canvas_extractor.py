...
async def get_canvas_data(table: str,
    ...
    async with DAPClient() as session:
        if query_type == "snapshot":
            ...
            query = IncrementalQuery(format=data_format, mode=mode, since=last_seen, until=None) 
        else:
        ...
        for i_object in query_object.objects:
            filename = await session.download_object(i_object, output_directory, decompress=True) # outputs in UTF-8 encoding
            ...
            with open(final_file, 'wb') as wfd:
                for file in filenames:
                    with open(file, 'rb') as fd:
                        ...
                        logger.info("Merged file: %s", final_file) #TODO: This also merges headers for CSV and TSV files, figure out how to resolve. Works fine for JSON
...
async def update_all(work_queue: asyncio.Queue, user_config: dict) -> None:
    ...
    while not work_queue.empty():
        ...
        try:
            logger.info("Task [%s] beginning Canvas data pull for table: %s.", table, table)
            ...
            logger.info("Task [%s] completed Canvas data pull for table: %s.", table, table)
...
async def main(user_config: dict) -> None:
    ...
    Credentials.create(client_id=user_config.dap_client_id, client_secret=user_config.dap_client_secret)
    ...
    tasks = [asyncio.create_task(update_all(work_queue, user_config)) for table in tables]
