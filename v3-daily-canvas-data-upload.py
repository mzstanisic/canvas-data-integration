...
async def update_course_sections():
    async with DAPClient() as session:
        ...
        with gzip.open(file_path, 'rb') as f_in:
            with open(os.path.join(prefix_path,'temp-course_sections.csv'), 'wb') as f_out:
                shutil.copyfileobj(f_in, f_out) 
        ...
        df2.drop(['value.integration_id','value.created_at','value.updated_at','value.workflow_state','value.sis_batch_id','value.start_at','value.end_at','value.sis_source_id','value.default_section','value.accepting_enrollments','value.restrict_enrollments_to_section_dates','value.nonxlist_course_id'],axis=1,inplace=True)
        df_merged = pd.concat([df1,df2])
        dupes_removed = df_merged.drop_duplicates("key.id",keep="last",inplace=False) 
        dupes_removed.to_csv(os.path.join(final_path,"canvas-course_sections.csv"), index=False)
async def update_enrollment_terms():
    async with DAPClient() as session:
            ...
            async with session.stream_resource(resource) as stream:
                async with aiofiles.open(file_path, "wb") as file:
                    ...
                    async for chunk in stream.iter_chunked(64 * 1024):
                        await file.write(chunk)
        with gzip.open(file_path, 'rb') as f_in:
            with open(os.path.join(prefix_path,'temp-enrollment_terms.csv'), 'wb') as f_out:
                shutil.copyfileobj(f_in, f_out) 
        #merge existing file with new extract
        ...
        df2.drop(['value.integration_id','value.created_at','value.updated_at','value.workflow_state','value.sis_batch_id','value.start_at','value.end_at','value.sis_source_id','value.grading_period_group_id'],axis=1,inplace=True)
        df_merged = pd.concat([df1,df2])
        dupes_removed = df_merged.drop_duplicates("key.id",keep="last",inplace=False) 
        dupes_removed.to_csv(os.path.join(final_path,"canvas-enrollment_terms.csv"), index=False)
async def update_enrollments():
    async with DAPClient() as session:
        ...
        df2.drop(['value.sis_batch_id','value.user_id','value.created_at','value.updated_at','value.workflow_state','value.role_id','value.start_at','value.end_at','value.completed_at','value.grade_publishing_status','value.associated_user_id','value.self_enrolled','value.limit_privileges_to_course_section','value.total_activity_time','value.last_attended_at','value.type'],axis=1,inplace=True)
        df_merged = pd.concat([df1,df2])
        dupes_removed = df_merged.drop_duplicates("key.id",keep="last",inplace=False) 
        dupes_removed.to_csv(os.path.join(final_path,"canvas-enrollments.csv"), index=False)
async def update_pseudonyms():
    async with DAPClient() as session:
        ...
        df2.drop(['value.deleted_at','value.integration_id','value.created_at','value.updated_at','value.workflow_state','value.account_id','value.sis_batch_id','value.unique_id','value.login_count','value.failed_login_count','value.last_request_at','value.last_login_at','value.current_login_at','value.last_login_ip','value.current_login_ip','value.authentication_provider_id','value.position'],axis=1,inplace=True)
        df_merged = pd.concat([df1,df2])
        dupes_removed = df_merged.drop_duplicates("key.id",keep="last",inplace=False) 
        dupes_removed.to_csv(os.path.join(final_path,"canvas-pseudonyms.csv"), index=False)
async def update_scores():
    async with DAPClient() as session:
        ...
        df2 = pd.read_csv(os.path.join(prefix_path,'temp-scores.csv'), dtype={'meta.ts': str})
        df2.drop(['value.created_at','value.updated_at','value.workflow_state','value.assignment_group_id','value.grading_period_id','value.final_score','value.unposted_current_score','value.unposted_final_score','value.current_points','value.unposted_current_points','value.final_points','value.unposted_final_points','value.override_score'],axis=1,inplace=True)
        df_merged = pd.concat([df1,df2])
        dupes_removed = df_merged.drop_duplicates("key.id",keep="last",inplace=False) 
        dupes_removed.to_csv(os.path.join(final_path,"canvas-scores2.csv"), index=False)
async def update_users():
    async with DAPClient() as session:
        ...
        df2.drop(['value.deleted_at','value.storage_quota','value.lti_context_id','value.created_at','value.updated_at','value.workflow_state','value.sortable_name','value.avatar_image_url','value.avatar_image_source','value.avatar_image_updated_at','value.short_name','value.last_logged_out','value.pronouns','value.merged_into_user_id','value.locale','value.time_zone','value.uuid','value.school_name','value.school_position','value.public'],axis=1,inplace=True)
        df_merged = pd.concat([df1,df2])
        dupes_removed = df_merged.drop_duplicates("key.id",keep="last",inplace=False) 
